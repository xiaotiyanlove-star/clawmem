...
- 🛡️ **隔离与安全** — 原生支持级联 `user_id` 的租户级存储物理隔离，内置 `AUTH_TOKEN` 轻量鉴权，防 SSRF 及大 Payload 攻击。
- ⚡ **高并发与缓存** — 内置 SQLite 语义缓存，并重构了 **Go-Channel 异步批处理写队列** 解决高频并发写锁死问题。
- 🔌 **全能 MCP 协议** — 内置 MCP Server，提供 `add_memory`, `search_memory`, `set_memory`, `delete_memory`, `get_preferences` 整套 Agent 记忆工具链。
- 💤 **梦境引擎 (Dream)** — 后台自动基于 JSON 结构化整合碎片化记忆。不仅提炼摘要，还会依据 `supersedes` 机制自动解决用户偏好冲突覆盖。
- 📊 **可视化大盘 (Dashboard)** — `/dashboard` 提供纯静态、高颜值的记忆用量红绿预警大屏与健康度监控。
...
*(如果配置了 `AUTH_TOKEN`，请在请求头携带 `X-API-KEY: <token>` 或 `Authorization: Bearer <token>`)*

### 存储 / 覆盖记忆

```bash
# 自动智能去重与覆盖 (推荐智能体使用，自动处理冲突与软删)
...
curl -H "X-API-KEY: your_auth_token" "http://localhost:8090/api/v1/memo/search?user_id=user-001&query=服务器IP&top_k=5"
```

### 可视化面板 (Dashboard)

启动服务后，使用浏览器直接访问即可查看物理节点的记忆治理大屏：
```
...
ClawMem 内置了 MCP Server 二进制（`clawmem-mcp`），可与所有 MCP 兼容客户端无缝集成。暴露了 5 个核心隔离工具：

- `add_memory`: 传统的无极增加工具。
- `search_memory`: 查找某用户下的片段。
- `set_memory`: 发起一次替换性质的知识库陈述设置。
- `get_preferences`: 列举某个 user_id 下的高优偏好法则。
- `delete_memory`: 通过特定 ID 抹除记忆痕迹。
